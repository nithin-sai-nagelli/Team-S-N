...
def select(a):
   sql= '''select distinct subcategory.subcat_id,subcategory.subcategory from cathassubcat,subcategory where cathassubcat.category_id=''' +str(a)+ '''and cathassubcat.subcat_id=subcategory.subcat_id limit ''' + str(900)
...
def selectb(a):
   sql= '''select distinct brand.brand_id,brand.brand from subcathasbrand,brand where subcathasbrand.subcat_id=''' +str(a)+ '''and subcathasbrand.brand_id=brand.brand_id limit ''' + str(900)
   cursor_obj.execute(sql)
   profiles=cursor_obj.fetchall()
...
def selectp(a):
   sql= '''select distinct product.no_of_likes,product.product_id,image from brandhaspdt,product,productlink where brandhaspdt.brand_id=''' +str(a)+ '''and brandhaspdt.product_id=product.product_id and product.product_id=productlink.product_id limit ''' + str(900)
   cursor_obj.execute(sql)
   profiles=cursor_obj.fetchall()
...
def selecti(a):
    sql='''select distinct actual_price,current_price,discount,no_of_likes,image,product.product_id,quantity from product,productlink where product.product_id='''+str(a)+'''and productlink.product_id='''+str(a)
    cursor_obj.execute(sql)
    profiles=cursor_obj.fetchall()
...
@app.route('/profile')
def profile():
    sql='''SELECT name,password,phone_no,email_id,address from customer where user_id='''+str(uid)
    cursor_obj.execute(sql)
    p=cursor_obj.fetchall()
...
def wl():
    s="'"+str(uid)+"'"
    sql='''SELECT distinct wlhaspdt.product_id,image from wlhaspdt,productlink where wlhaspdt.user_id='''+s+''' and wlhaspdt.product_id=productlink.product_id'''
    cursor_obj.execute(sql)
    profiles=cursor_obj.fetchall()
    print(uid)
    print(profiles)
...
def add_to_wl(a):
    s="'"+str(uid)+"'"
    b="'"+str(a)+"'"
    sql='''select * from wlhaspdt where user_id='''+s+''' and product_id='''+b
    cursor_obj.execute(sql)
    p=cursor_obj.fetchall()
    if(len(p)>0):
        return redirect('/wl')
